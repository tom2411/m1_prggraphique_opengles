 ...
 */
    ...
    static float rondCoords[] = {
            -1.0f,  1.0f, 0.0f,
            -1.0f, -1.0f, 0.0f,
            1.0f, -1.0f, 0.0f,
            1.f, 1.f, 0.0f };
    // Le tableau des couleurs
    static float rondColors[] = {
             0.0f,  0.0f, 1.0f, 1.0f,
             0.0f,  0.0f, 1.0f, 1.0f,
             0.0f,  0.0f, 1.0f, 1.0f,
             0.0f,  0.0f, 1.0f, 1.0f };
    ...
    public Rond(float[] Pos) {
        ...
        for (int i = 0; i < rondCoords.length-1; i+=3) {
            rondCoords[i] += Position[0];
            rondCoords[i+1] += Position[1];
        }
        // initialisation du buffer pour les vertex (4 bytes par float)
        ByteBuffer bb = ByteBuffer.allocateDirect(rondCoords.length * 4);
        bb.order(ByteOrder.nativeOrder());
        vertexBuffer = bb.asFloatBuffer();
        vertexBuffer.put(rondCoords);
        vertexBuffer.position(0);


        // initialisation du buffer pour les couleurs (4 bytes par float)
        ByteBuffer bc = ByteBuffer.allocateDirect(rondColors.length * 4);
        bc.order(ByteOrder.nativeOrder());
        colorBuffer = bc.asFloatBuffer();
        colorBuffer.put(rondColors);
